  ...
  try {
    ...
    if (config.mail_client_id && config.mail_client_secret && config.mail_refresh_token) {
      ...
      oauth2Client.setCredentials({
        ...
        expiry_date: config.token_expires_at ? new Date(config.token_expires_at).getTime() : null
      });

      // Get fresh access token if needed
      if (!config.mail_access_token || (config.token_expires_at && new Date(config.token_expires_at) <= new Date())) {
        const { token } = await oauth2Client.getAccessToken();
    ...
    } else if (config.smtp_host && config.smtp_user && config.smtp_password) {
      // SMTP configuration
      transporter = nodemailer.createTransporter({
      ...
      throw new Error('Invalid email configuration: Neither OAuth2 nor SMTP credentials provided');
    }
