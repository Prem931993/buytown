export async function seed(knex) {
  // Insert vehicle management data with onConflict to handle duplicates
  await knex('byt_vehicle_management')
    .insert([
  ...
  // Insert delivery settings data with onConflict to handle duplicates
  await knex('byt_delivery_settings')
    .insert([
      { center_point: 'RS Puram', delivery_radius_km: 10, is_active: true }
    ])
    .onConflict('center_point')
    .ignore();
};
